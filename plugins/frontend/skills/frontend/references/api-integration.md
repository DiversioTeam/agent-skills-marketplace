...
Examples:
- If OpenAPI or Swagger exists locally, use it instead of asking for backend
  code.
- If Bruno docs exist locally, use them instead of reverse-engineering
  endpoints from components.
- If the repo is a monolith with frontend and backend together, detect the
  local backend path and use it.
