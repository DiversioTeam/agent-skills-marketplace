...
|---------|-------------------|
| [`ci-status`](./ci-status) | `/ci`, `/ci-detail`, `/ci-logs`, auto-watch, and LLM tools with fresh checkout-scoped lookup, exact GitHub job selection, real CircleCI console output, and explicit errors for ambiguous jobs or unavailable logs |
